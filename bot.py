...
def generar_reporte_periodo(registros, titulo):
    ...
    c.setFont("Helvetica", 10)
    c.drawString(50, 735, f"Generado: {datetime.now().strftime('%d/%m/%Y %H:%M')}")
...
async def ver_deudores(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not kb: await update.message.reply_text("✅ No hay deudas.")
...
async def manejador_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if d[0] == "p":
        t, c, m = d[1], d[2], d[3]
        if t != 'fia':
            supabase.table("ventas").insert({"producto": f"PAGO {t.upper()}", "precio": int(m), "cliente": c.lower(), "tipo": "pago", "id_telegrama": u, "fecha": datetime.now().isoformat()}).execute()
            txt = f"✅ **PAGADO: {c.upper()}** (${m})"
        else: txt = f"⏳ **FIADO: {c.upper()}** (${m})"
        ...
        if datos:
            ...
            with open(path, 'rb') as f: await q.message.reply_document(document=f)
            os.remove(path)
        ...
        if res.data:
            ...
            with open(path, 'rb') as f: await q.message.reply_document(document=f, caption=f"Reporte {tipo} generado con éxito.")
            os.remove(path)
        else: await q.message.reply_text("No hay datos para este periodo.")
...
async def gestionar_mensajes(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if u in esperando_pago_deuda:
        ...
        except: await update.message.reply_text("Enviá solo números.")
        return
    ...
    if u in esperando_nombre:
        ...
        for i in carritos[u]:
            supabase.table("ventas").insert({"producto": f"{i['cantidad']}x {i['producto']}", "precio": i['subtotal'], "cliente": c.lower(), "tipo": "venta", "id_telegrama": str(u), "fecha": datetime.now().isoformat()}).execute()
    ...
    if t == '🛒 Carrito Nuevo': carritos[u] = []; await update.message.reply_text("Carrito abierto.")
    elif t == '🧾 Finalizar Ticket':
        if u in carritos and carritos[u]: esperando_nombre[u] = True; await update.message.reply_text("👤 ¿Nombre del cliente?")
        else: await update.message.reply_text("El carrito está vacío.")
    elif t == '📉 Lista Deudores': await ver_deudores(update, context)
    elif t == '📊 Caja del Día': await mostrar_caja_del_dia(update, context)
    elif t == '📄 Reporte PDF': 
    ...
    elif t == '🔄 Reiniciar Menú': await start(update, context)
    elif u in carritos:
        n = re.findall(r'\d+', t)
        p = re.findall(r'[a-zA-ZñÑ]+', t)
        if n:
            can, pre = (int(n[0]), int(n[-1])) if len(n) >= 2 else (1, int(n[0]))
            carritos[u].append({"producto": " ".join(p) or "Prod", "cantidad": can, "precio": pre, "subtotal": can * pre})
            await update.message.reply_text(f"✅ Agregado. Total: ${sum(i['subtotal'] for i in carritos[u])}")
